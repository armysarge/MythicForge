        ...
        DataLoader.pCacheAndGet(type, source, hash).then(function(data) {
            ...
            inlineContent.inlineContentHtml(data, type).then(function(theContent) {
                if (theContent != "") {
                    ...
                    } else if (type.split(".")[0] == "items") {
                        inlineContent.itemStatsHTML(data.name, source);
                    } else {
        ...
        that.getMonsterByName(name,source).then(function(monster){
            if(monster){
                ...
                <div class="StatBlock">
                    ...
                    <div class="windowIcon createMonsterStory ${(!getUserSettings().using_ai)?'disabled':""} ${(monsterPicture == "")?"bottomIcon":""}" title='${(getUserSettings().using_ai)?'Create a random story for this monster' : 'AI features disabled'}'></div>
